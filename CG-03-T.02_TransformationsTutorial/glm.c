...
#ifdef WIN32
	#pragma warning(disable : 4996) /* CHANGED: disable VS2005 'strdup' deprecated warning */
#endif
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 'f':               /* face */
                v = n = t = 0;
                fscanf(file, "%s", buf);
                /* can be one of %u, %u//%u, %u/%u, %u/%u/%u %u//%u */
                if (strstr(buf, "//")) {
                    /* v//n */
                    sscanf(buf, "%u//%u", &v, &n);
                    fscanf(file, "%u//%u", &v, &n);
...
#if 0
    printf("glmLinearTexture(): generated %u linear texture coordinates\n",
        model->numtexcoords);
#endif
}
...
if (model->numnormals) {
    ...
    printf("glmOptimize(): %u redundant normals.\n",
...
if (model->numtexcoords) {
    ...
    printf("glmOptimize(): %u redundant texcoords.\n",

...
///////////////////////////////////////////////////////////////////////////////////////////////////
{
	...
	//glViewport(0, 0, (GLsizei)640, (GLsizei)640); // w/h
	//GLfloat aspect = (GLfloat)640 / (GLfloat)640; // w/h
	//glm::mat4 perspective = glm::perspective(79.0f * (glm::pi<float>() / 180.0f), aspect, 0.1f, 240.0f);
	//glm::mat4 camera = glm::lookAt(glm::vec3(2.0, 2.0, 2.0), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0));

	model = model * view;
	//model = perspectiveView ? model*perspective*camera : model;
...
void glutReshapeCB(int w, int h){
	// get and setup projection matrix
	GLint location = glGetUniformLocation(PROGRAM_ID, "matProjection");
	glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(projection));
	glViewport(0, 0, (GLsizei)w, (GLsizei)h); // w/h
	GLfloat aspect = (GLfloat)w / (GLfloat)h; // w/h
	glm::mat4 camera = glm::lookAt(glm::vec3(10.0, 10.0, 10.0), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0));

	// Perspective
	glm::mat4 perspective = glm::perspective(79.0f * (glm::pi<float>() / 180.0f), aspect, 0.1f, 240.0f);
	
	// Orthographic
	GLfloat left = -10.0f, right = 10.0f;
	GLfloat bottom = -10.0f, top = 10.0f;
	GLfloat znear = 0.1f, zfar = 256.0f;
	if (aspect > 1.0f) { left *= aspect; right *= aspect; }
	else { bottom /= aspect; top /= aspect; }
	glm::mat4 orthographic = glm::ortho(left, right, bottom, top, znear, zfar);
	
	projection = perspectiveView ? perspective*camera : orthographic*camera;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
{
	switch (item)
	{
		...
		case 3:
		{
			initMenuChange(2, "Switch to Perspective Projection", 4);
			perspectiveView = false;
			glutReshapeCB(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT));
			break;
		}
		case 4:
		{
			initMenuChange(2, "Switch to Orthogonal Projection", 3);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
{
	...
	MENU_ENTRY_STR[2] = "Switch to Orthographic Projection";
...
{
	perspectiveView = perspectiveView ? false : true;
